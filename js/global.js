...
function fShowID(trackinghome) {
	...
	if (trackinghome == "HAWB") {
	...
	} else if (trackinghome == "shipmentNbr") {
		if (shipmentTrackID.length === 0) {
			alert("The shipment reference is not recognized. Please search again");
			document.getElementById("hometrackinput").focus();
			return false;
		}
	} else {
		if (!TNT_Validate(shipmentTrackID)) {
			...
			document.getElementById("hometrackinput").focus();
			return false;
			// Validation for track lenght (only 9 or 11 symbols)
			}
		// else if (shipmentTrackID.length != 9 && shipmentTrackID.length != 11) {
		// 	alert("The shipment reference is not recognized1. Please search again using a reference in the following format: SIN716274");
		// 	document.getElementById("hometrackinput").focus();
		// 	return false;
		// }
	}
	...
	function TNT_Validate(refNo) {
		...
		// Start validation with format SIN123456 (first 3 symbols are string, another - numeric)
	...
	window.open( url_track.url + trackinghome + '&refNo=' + shipmentTrackID, 'formresult', 'scrollbars=yes,menubar=no,height=600,width=800,resizable=yes,toolbar=no,status=yes');
}
;(function ($) {
	...
	$('.subscribe-form__close').on('click', function (e) {
		e.preventDefault();
		$('.subscribe-form').fadeOut();
		localStorage.setItem('firstClose', '1');
	});
	if (localStorage.getItem('firstClose') !== "1") {
	...
	$(document).ready(function () {
		...
		//resources page load ajax load function
		let postsLength = $('.posts-loop .columns').length;
		function loadMoreResources(actionName,offset){
			let select = $('.resources-select'),
				loadMoreButton = $('#load-more-resources'),
				postsContainer = $('.posts-loop');

			//check if we still loading
			if (loadMoreButton.hasClass('is-loading')) return;

			loadMoreButton.addClass('is-loading').text('Loading...');
			postsContainer.addClass('in-loading');

			let data = {
				'action': actionName,
				'cat': parseInt(select.val()),
				'page_id': parseInt(select.data('page-id')),
				'offset': offset,
				'nonce': myajax.nonce
			};
			...
			$.ajax({
				...
				success: function (res) {
					if (res) {
						//check for replace all results or just add from load more
						if( offset ){
							postsContainer.find('.columns:last-child').after(res)
							postsLength += 15;
						} else {
							postsLength = 15;
							postsContainer.html('').html(res)
						}

						postsContainer.removeClass('in-loading');
						loadMoreButton.text('load more');

						//check for load more button visibility
						$('.posts-loop .columns').length < postsLength ? loadMoreButton.hide() : loadMoreButton.show();
					} else {
		...
		//resources select change
		$('.resources-select').on('change', function (){
			loadMoreResources('resources_loading',0);
		});

		//load more on resources page
		$(document).on('click','#load-more-resources',function (){
			loadMoreResources('resources_loading', $('.posts-loop .columns').length);
		});
  ...
  return !!pattern.test(str);
		...
		$('body').on('click', 'a', function (event) {
			...
			if (!validURL($(this).attr('href')) && this.pathname == window.location.pathname &&
